...
public class ServletAction extends HttpServlet {
	...
	public void processAction(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		if(session.getAttribute("user_role")==null){
			try {
				processError(request, response,100,"session超时，请重新登录系统！");
	...
	public void showDebug(String msg){
		System.out.println("["+(new SimpleDateFormat("yyyy-MM-dd")).format(new Date())+"]["+module+"/"+sub+"/ServletAction]"+msg);
	}
	...
	public void getRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        showDebug("sort index:"+sortIndex);
		String orderBy=request.getParameter("order_by");
        showDebug("order by:"+orderBy);
		...
		if(existResultset.equals("1")){
			...
			jsonObj=forumDao.getRecord(query);
	...
	public void getRecord1(HttpServletRequest request, HttpServletResponse response) throws Exception {
		HttpSession session = request.getSession();
		String dbName=(String)session.getAttribute("unit_db_name");
		String action = request.getParameter("action");
		String id=request.getParameter("id");
...
//		jsonObj.put("user_name",userName);
	...
	public void addRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(dbName!=null){
			...
			forum.setAction(action);
			forum.setDbName(dbName);
			forum.setParentId(projectId);
			forum.setTitle(title);
			forum.setContent(content);
			forum.setLimitTime(limitTime);
			forum.setUserId(userId);
			forum.setCreator(creator);
			forum.setCreateTime(createTime);
			jsonObj=forumDao.addRecord(forum);
	...
	public void modifyRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(id!=null && dbName!=null){
			...
			forum.setCreator(creator);
			forum.setCreateTime(createTime);
			jsonObj=forumDao.modifyRecord(forum);
	...
	public void deleteRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(ids!=null && dbName!=null){
			...
			/*----------------------------------------数据获取完毕，开始和数据库交互*/
			forumDao forumDao=new forumDao();
			jsonObj=forumDao.deleteRecord(action,dbName,ids,creator,createTime);
	...
	public void setRecordTop(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(dbName!=null){
			...
			String createTime=(new SimpleDateFormat("yyyy-MM-dd")).format(new Date());
			forumDao forumDao=new forumDao();
			jsonObj=forumDao.setRecordTop(action,dbName,type,userId,id);
			jsonObj.put("user_id",userId);
			jsonObj.put("user_name",userName);
		}
		boolean isAjax=true;
	...
	public void exportRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		JSONObject jsonObj = null;
		jsonObj=(JSONObject)session.getAttribute(module+"_"+sub+"_get_record_result");
		showDebug("hhhhhhhhhhhhhhhhhh"+jsonObj.toString());
		...
		response.addHeader("Content-Disposition", "attachment;filename=forum.xls");//文件临时保存在demo.xls中
		out.flush();
		out.close();
	}
